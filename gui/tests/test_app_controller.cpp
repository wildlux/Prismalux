...
private slots:
    ...
    void genLettaraOutputContainsModel() {
        ...
        auto* lista = m_page->findChild<QListWidget*>("offerteList");
        if (!lista || lista->count() == 0) { QSKIP("Lista offerte vuota — skip"); }
        lista->setCurrentRow(0);
        QApplication::processEvents();

        // Trova il pulsante "Genera Lettera" (non "Cover Letter")
        QPushButton* genBtn = nullptr;
        for (auto* b : m_page->findChildren<QPushButton*>())
            if (b->text().contains("Lettera con AI")) { genBtn = b; break; }
        if (!genBtn) { QSKIP("Pulsante genera lettera non trovato — skip"); }

        // Trova il log email (objectName "chatLog" — m_lavoroLog)
        auto* log = m_page->findChild<QTextEdit*>("chatLog");
        if (!log) { QSKIP("Log email non trovato — skip"); }

        log->clear();
        QVERIFY2(genBtn->isEnabled(),
                 qPrintable(QString("Pulsante '%1' disabilitato — non cliccabile").arg(genBtn->text())));
        genBtn->click();   // click() emette clicked() senza dipendere dalla geometria
        QApplication::processEvents();

        // L'header deve contenere "Modello:" con il nome del modello
        QVERIFY2(log->toPlainText().contains("Modello"),
                 qPrintable(QString("Output deve contenere 'Modello'. Log: '%1' | Lista currentRow: %2 | btn: '%3'")
                     .arg(log->toPlainText().left(200))
                     .arg(lista->currentRow())
                     .arg(genBtn->text())));
    }
