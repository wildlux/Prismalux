...
| `Vision3D` | `test_vision3d` | 19 PASS — CAT-A costruzione, UI, porta default `P::kVision3DPort`, setter, combo interfacce (LAN 192.168 prima, no docker/virbr, loopback in coda), galleria `v3dGallery` + combo VLM `v3dVlmCombo`, scena 3D `v3dScene` (addShot dedup/clear) + ricostruzione `v3dReconBtn`/`v3dSessionCombo`, posa manuale `v3dPoseHead`/`v3dPosePitch` (wrapping, disabilitati senza selezione) + `v3dDistribBtn`, guida `v3dHelpBtn` + sonda COLMAP a caldo `v3dRecheckBtn`/`v3dReconHint`, contatore foto/qualità `v3dPhotoReq` (low≥10 medium≥20 high≥40); CAT-B stop idempotente, distruzione pulita, start porta privilegiata; CAT-C e2e server HTTPS reale con curl su bind 127.0.0.1 (GET pagina, 404, POST /upload salva jpg+json, GET /download obj/mtl/ply) — QSKIP senza curl/openssl. Pipeline recon 3 passi: sparse → PLY → OBJ+MTL colorati via `Tools/scripts/ply_to_obj.py` |
