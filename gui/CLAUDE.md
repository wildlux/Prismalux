...
P::root()              // CMAKE_SOURCE_DIR/.. = cartella Prismalux/
P::modelsDir()         // cartella .gguf (models/ > llama_cpp_studio/models/)
...
**Nota path**: `P::root()` punta a `Prismalux/`. Tutte le sottocartelle sono dirette:
`P::root() + "/models"`, `P::root() + "/llama_cpp_studio"`, `P::root() + "/RAG"`, ecc.
...
| VRAM benchmark | ✅ | binario vram_bench in Prismalux/ |
...
**Causa**: path `P::root() + "/C_software"` generava un doppio nesting.
Il Makefile non era nella dir cercata; make falliva con codice 2.
**Fix**: path corretti a `P::root()` (= `Prismalux/`). Aggiunto check `QFileInfo::exists(Makefile)`;
...
`P::root()` = `Prismalux/` → `P::root() + "/RAG"` = `Prismalux/RAG/`.  
