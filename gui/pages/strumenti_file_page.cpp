...
QWidget* StrumentiFilePage::buildFileSearchTab()
{
    ...
    connect(btnBrowse,        &QPushButton::clicked,
            this, &StrumentiFilePage::onFileSearchBrowseClicked);
    connect(m_fileSearchBtn,  &QPushButton::clicked,
            this, &StrumentiFilePage::onFileSearchSearchClicked);
    connect(m_fileSearchAiBtn,&QPushButton::clicked,
            this, &StrumentiFilePage::onFileSearchAiClicked);
...
QWidget* StrumentiFilePage::buildWikiTab()
{
    ...
    m_wikiActRow = new QWidget(panel);
    auto* actLay = new QHBoxLayout(m_wikiActRow);
    ...
    m_wikiNam = new QNetworkAccessManager(this);

    connect(btnFetch,        &QPushButton::clicked,
            this, &StrumentiFilePage::onWikiFetchClicked);
    connect(m_wikiElaboraBtn,&QPushButton::clicked,
            this, &StrumentiFilePage::onWikiElaboraClicked);
...
QWidget* StrumentiFilePage::buildDatiTab()
{
    ...
    connect(fileBtn,    &QPushButton::clicked,
            this, &StrumentiFilePage::onDatiFileBtnClicked);
    connect(previewBtn, &QPushButton::clicked,
            this, &StrumentiFilePage::onDatiPreviewBtnClicked);
    connect(analyzeBtn, &QPushButton::clicked,
            this, &StrumentiFilePage::onDatiAnalyzeBtnClicked);
...
QWidget* StrumentiFilePage::buildPdfTab()
{
    ...
    connect(fileBtn,    &QPushButton::clicked,
            this, &StrumentiFilePage::onPdfFileBtnClicked);
    connect(btnAnalyze, &QPushButton::clicked,
            this, &StrumentiFilePage::onPdfAnalyzeBtnClicked);
...
QWidget* StrumentiFilePage::buildWordTab()
{
    ...
    m_wordOutputEdit = new QTextEdit(panel);
    ...
    connect(fileBtn,    &QPushButton::clicked,
            this, &StrumentiFilePage::onWordFileBtnClicked);
    connect(btnAnalyze, &QPushButton::clicked,
            this, &StrumentiFilePage::onWordAnalyzeBtnClicked);

    return panel;
}

/* ══════════════════════════════════════════════════════════════
   SLOT IMPLEMENTATIONS — File Search
   ══════════════════════════════════════════════════════════════ */

void StrumentiFilePage::onFileSearchBrowseClicked()
{
...
void StrumentiFilePage::onFileSearchSearchClicked()
{
    ...
    m_fileSearchBtn->setEnabled(false);
    m_fileSearchAiBtn->setEnabled(false);
    m_fileSearchOut->setPlainText(
...
void StrumentiFilePage::onFileSearchProcFinished(int exitCode, QProcess::ExitStatus)
{
    const QString query = m_fileSearchQuery->text().trimmed();
    const QString raw   = QString::fromUtf8(m_fileSearchProc->readAll()).trimmed();
    m_fileSearchProc->deleteLater();
    m_fileSearchProc = nullptr;
    m_fileSearchBtn->setEnabled(true);
    ...
    m_fileSearchAiBtn->setEnabled(true);
}

void StrumentiFilePage::onFileSearchTimeout()
{
    if (m_fileSearchProc) {
        m_fileSearchProc->kill();
        m_fileSearchBtn->setEnabled(true);
...
void StrumentiFilePage::onFileSearchAiClicked()
{
    ...
    connect(m_ai, &AiClient::token,    this, &StrumentiFilePage::onFileSearchAiToken);
    connect(m_ai, &AiClient::finished, h, [h](const QString&){ h->deleteLater(); });
    connect(m_ai, &AiClient::error,    this, &StrumentiFilePage::onFileSearchAiError);
    m_ai->chat(P::prependKnowledge(sys), ctx + "\nDomanda: " + query);
}

void StrumentiFilePage::onFileSearchAiToken(const QString& t)
{
...
/* ══════════════════════════════════════════════════════════════
   SLOT IMPLEMENTATIONS — Wiki
   ══════════════════════════════════════════════════════════════ */

void StrumentiFilePage::onWikiActBtnClicked()
{
    if (m_wikiExtract.isEmpty()) return;
    auto* ab = qobject_cast<QPushButton*>(sender());
    if (!ab) return;
    ...
    auto* h = new QObject(this);
    connect(m_ai, &AiClient::token,    this, &StrumentiFilePage::onWikiActAiToken);
    connect(m_ai, &AiClient::finished, h, [h](const QString&){ h->deleteLater(); });
    connect(m_ai, &AiClient::error,    this, &StrumentiFilePage::onWikiActAiError);
    m_ai->chat(P::prependKnowledge(sys),
            "[Articolo Wikipedia]\n" + m_wikiExtract + "\n\n" + prompt);
}

void StrumentiFilePage::onWikiFetchClicked()
{
    ...
    auto* reply = m_wikiNam->get(req);
    connect(reply, &QNetworkReply::finished,
            this, &StrumentiFilePage::onWikiReplyFinished);
}

void StrumentiFilePage::onWikiReplyFinished()
{
    ...
    const QString q    = m_wikiLastQuery;
    const QString lang = m_wikiLastLang;
    ...
    /* Enable elabora + action buttons */
    m_wikiElaboraBtn->setEnabled(true);
    if (m_wikiActRow) {
        const auto btns = m_wikiActRow->findChildren<QPushButton*>("wikiActBtn");
        for (auto* b : btns) b->setEnabled(true);
    }
}

void StrumentiFilePage::onWikiElaboraClicked()
{
    if (m_wikiExtract.isEmpty()) return;
    const QString sys =
        "Sei un assistente esperto. Usa le informazioni dell\xe2\x80\x99" "articolo Wikipedia "
    ...
    m_wikiContent->append("\n\n--- AI ---\n");
    auto* h = new QObject(this);
    connect(m_ai, &AiClient::token,    this, &StrumentiFilePage::onWikiAiToken);
    connect(m_ai, &AiClient::finished, h, [h](const QString&){ h->deleteLater(); });
    connect(m_ai, &AiClient::error,    this, &StrumentiFilePage::onWikiAiError);
    const QString question = m_wikiQuery->text().trimmed();
    m_ai->chat(P::prependKnowledge(sys),
            "[Articolo Wikipedia]\n" + m_wikiExtract + "\n\n" + question);
}

void StrumentiFilePage::onWikiAiToken(const QString& t)
{
...
void StrumentiFilePage::onWikiActAiToken(const QString& t)
{
    QTextCursor c = m_wikiContent->textCursor();
    c.movePosition(QTextCursor::End);
    m_wikiContent->setTextCursor(c);
...
/* ══════════════════════════════════════════════════════════════
   SLOT IMPLEMENTATIONS — Dati AI
   ══════════════════════════════════════════════════════════════ */

void StrumentiFilePage::onDatiFileBtnClicked()
{
    ...
    if (ext == "csv" || ext == "tsv" || ext == "txt") {
        ...
        /* Store tmpScript path in the process object via property for use in slot */
...
void StrumentiFilePage::onDatiXlsProcFinished(int code, QProcess::ExitStatus)
{
    auto* proc = qobject_cast<QProcess*>(sender());
    if (!proc) return;
    const QString text = QString::fromUtf8(
        proc->readAllStandardOutput()).trimmed();
    const QString scriptPath = proc->property("tmpScriptPath").toString();
    auto* tmpScript = qobject_cast<QTemporaryFile*>(
        proc->property("tmpScriptObj").value<QObject*>());
    proc->deleteLater();
    if (tmpScript) { tmpScript->remove(); tmpScript->deleteLater(); }
...
void StrumentiFilePage::onDatiPreviewBtnClicked()
{
...
void StrumentiFilePage::onDatiAnalyzeBtnClicked()
{
    if (m_datiCsvText.isEmpty()) {
    ...
    auto* h = new QObject(this);
    connect(m_ai, &AiClient::token,    this, &StrumentiFilePage::onDatiAiToken);
    connect(m_ai, &AiClient::finished, h, [h](const QString&){ h->deleteLater(); });
    connect(m_ai, &AiClient::error,    this, &StrumentiFilePage::onDatiAiError);

    m_ai->chat(sys,
        "Dati CSV da analizzare:\n\n```csv\n" + csv + "\n```");
}

void StrumentiFilePage::onDatiAiToken(const QString& t)
{
...
/* ══════════════════════════════════════════════════════════════
   SLOT IMPLEMENTATIONS — PDF
   ══════════════════════════════════════════════════════════════ */

void StrumentiFilePage::onPdfFileBtnClicked()
{
    const QString path = QFileDialog::getOpenFileName(
...
void StrumentiFilePage::runPdfAnalysis(const QString& text)
{
    ...
    m_pdfOutput->clear();
    auto* h = new QObject(this);
    connect(m_ai, &AiClient::token,    this, &StrumentiFilePage::onPdfAiToken);
    connect(m_ai, &AiClient::finished, h, [h](const QString&){ h->deleteLater(); });
    connect(m_ai, &AiClient::error,    this, &StrumentiFilePage::onPdfAiError);
    QString doc = text;
    if (doc.size() > 14000)
        doc = doc.left(14000) + "\n... [troncato per limiti di contesto]";
...
void StrumentiFilePage::onPdfAnalyzeBtnClicked()
{
...
void StrumentiFilePage::onPdfAiToken(const QString& t)
{
...
/* ══════════════════════════════════════════════════════════════
   SLOT IMPLEMENTATIONS — Word/Testo
   ══════════════════════════════════════════════════════════════ */

void StrumentiFilePage::onWordFileBtnClicked()
{
    const QString path = QFileDialog::getOpenFileName(
        this,
        "Seleziona file",
    ...
    m_wordFileLbl->setText(QFileInfo(path).fileName());
    m_wordFileContent.clear();
    m_wordOutputEdit->clear();
    ...
    if (ext == "docx") {
        ...
        if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
            QTextStream ts(&f);
            m_wordFileContent = ts.readAll();
            m_wordOutputEdit->setPlainText(
                "\xe2\x9c\x85  File caricato ("
                + QString::number(m_wordFileContent.size()) + " caratteri). "
...
void StrumentiFilePage::onWordDocxProcFinished(int code, QProcess::ExitStatus)
{
    auto* proc = qobject_cast<QProcess*>(sender());
    if (!proc) return;
    const QString text = QString::fromUtf8(
...
void StrumentiFilePage::onWordOdtProcFinished(int code, QProcess::ExitStatus)
{
    auto* proc = qobject_cast<QProcess*>(sender());
    if (!proc) return;
    const QString text = QString::fromUtf8(
...
void StrumentiFilePage::onWordAnalyzeBtnClicked()
{
    ...
    auto* h = new QObject(this);
    connect(m_ai, &AiClient::token,    this, &StrumentiFilePage::onWordAiToken);
    connect(m_ai, &AiClient::finished, h, [h](const QString&){ h->deleteLater(); });
    connect(m_ai, &AiClient::error,    this, &StrumentiFilePage::onWordAiError);
    m_ai->chat(sys, "DOCUMENTO:\n\n" + text);
}

void StrumentiFilePage::onWordAiToken(const QString& t)
{
    m_wordOutputEdit->moveCursor(QTextCursor::End);
    m_wordOutputEdit->insertPlainText(t);
}
