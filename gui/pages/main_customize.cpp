...
void PersonalizzaPage::onProc2ReadyRead() {
    ...
    const QString chunk = QString::fromLocal8Bit(m_proc2->readAll());
    m_buildOutputBuf += chunk;
    m_llamaLog->moveCursor(QTextCursor::End);
    m_llamaLog->insertPlainText(chunk);
    m_llamaLog->ensureCursorVisible();
}
...
void PersonalizzaPage::onProc2Finished(int code, QProcess::ExitStatus) {
    ...
    m_proc2 = nullptr;
    if (code != 0) {
        const bool vulkanMissing = m_buildOutputBuf.contains("glslc", Qt::CaseInsensitive)
                                || m_buildOutputBuf.contains("Could NOT find Vulkan", Qt::CaseInsensitive);
        if (vulkanMissing && m_buildGpuArgs.contains("-DGGML_VULKAN=ON")) {
            if (m_llamaLog)
                m_llamaLog->append("\n\xe2\x9a\xa0  Vulkan non disponibile (glslc mancante) "
                                   "\xe2\x80\x94 compilazione senza GPU Vulkan\n"
                                   "\xe2\x9a\x99  cmake configure (CPU)...\n");
            m_buildGpuArgs.removeAll("-DGGML_VULKAN=ON");
            m_buildGpuArgs.append("-DGGML_VULKAN=OFF");
            m_buildOutputBuf.clear();
