...
private:
    ...
    void     openQrDialog(QPushButton* parent, const QString& url,
                          ...
                          const QString& note);
    void     clientTableAddRow(const QString& ip, const QString& deviceName = {});
    void     clientTableRemoveRow(const QString& ip);
    static QString readMacForIp(const QString& ip);

    /* Lista persone autorizzate (nome, data) */
    QTableWidget* m_accessListTable = nullptr;
    void          loadAccessList();
    void          saveAccessList();
...
private slots:
    ...
    void onLanClientConnected(const QString& addr, const QString& deviceName);
