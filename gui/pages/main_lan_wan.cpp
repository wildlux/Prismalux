...
void LanWanPage::onTokenAutoGenerated(const QString& token)
{
    ...
    if (m_lanStatusLbl)
        m_lanStatusLbl->setToolTip(
            tr("\xf0\x9f\x94\x91 Token generato automaticamente \xe2\x80\x94 copialo nell'app Android"));
}
...
QWidget* LanWanPage::buildLanAndroidTab()
{
    ...
    auto* titleLbl = new QLabel(
        tr("<b>\xf0\x9f\x93\xb1  Server LAN per Android</b>"), tab);
    ...
    auto* clientGroup = new QGroupBox(
        tr("\xf0\x9f\x94\x92  Controllo accessi al Server Locale"), leftW);
    ...
    auto* qrAppHintLbl = new QLabel(
        tr("<small>Impostazioni \xe2\x86\x92 \xf0\x9f\x93\xb7 Scansiona QR</small>"),
        qrAppCol);
    ...
    auto* qrConnectBtn = new QPushButton(
        tr("\xf0\x9f\x93\xb1  QR Connetti (schermo intero)"), scrollW);
    ...
    m_lanWebBtn = new QPushButton(
        tr("\xf0\x9f\x8c\x90  Apri Chat Web nel browser"), scrollW);
    ...
    m_adbInstallBtn = new QPushButton(
        tr("\xf0\x9f\x94\x8c  Installa APK via USB  (adb)"), scrollW);
