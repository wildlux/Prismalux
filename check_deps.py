...
def _cpu_perc_reale() -> float:
    ...
    Aggiorna il valore solo se sono passati almeno 0.5 s dall'ultima misura.
    Su Windows/macOS usa il load average come fallback.
    """
    ...
    try:
        ...
        pass
    # Fallback: load average (Windows / macOS / /proc non disponibile)
    try:
...
def stampa_header(con, res: dict | None = None) -> None:
    """
    ...
    try:
        ...
        g.add_row(logo, r)
        con.print(Panel(g, border_style="bold yellow", padding=(0, 2)))
    except Exception:
