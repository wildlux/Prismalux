...
public:
    ...
    /* -- Token Bearer per LAN server Prismalux (porta 11500) -- */
    void    setToken(const QString& t)  { m_token = t; }
    QString bearerToken() const         { return m_token; }
    ...
    /* -- Streaming chat ------------------------------------------
       ...
       @param sys     System prompt (pu? essere vuoto)
       @param msg     Messaggio utente
       @param history Turni precedenti [{role,content},...] (pu? essere vuoto)
    ...
    /* -- Chat multimodale con immagine (modelli vision) -- */
    quint64 chatWithImage(const QString& sys, const QString& msg,
    ...
    void setLocalMode(bool on) {
        if (m_localMode != on) { m_localMode = on; emit localModeChanged(on); }
    }
    bool isLocalMode()    const { return m_localMode && m_localLlm; }
    LocalLlmClient* localLlm() const { return m_localLlm; }
