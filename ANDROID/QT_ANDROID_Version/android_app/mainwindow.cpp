...
#include "pages/pipeline_page.h"
#include "pages/sfida_page.h"
#include "pages/cron_page.h"
#include "pages/editor_page.h"
#include "pages/ricerca_mob_page.h"
#include "pages/collab_page.h"
#include "pages/appcontroller_page.h"
    ...
    : QMainWindow(parent)
{
    ...
    /* A-1 — PipelinePage: Byzantino + Agente Autonomo */
    m_pipelinePage = new PipelinePage(m_ai, this);
    m_stack->addWidget(m_pipelinePage);   // indice 26

    /* A-2 — SfidaPage: quiz gamificato AI-generated */
    m_sfidaPage = new SfidaPage(m_ai, this);
    m_stack->addWidget(m_sfidaPage);      // indice 27

    /* A-3 — CronPage: scheduler task periodici */
    m_cronPage = new CronPage(m_ai, this);
    m_stack->addWidget(m_cronPage);       // indice 28

    /* A-6 — EditorPage: editor testo + REPL Python */
    m_editorPage = new EditorPage(m_ai, this);
    m_stack->addWidget(m_editorPage);     // indice 29

    /* A-5 — RicercaMobPage: Ricerca strutturata + Grafo RAG */
    m_ricercaMobPage = new RicercaMobPage(m_ai, this);
    m_stack->addWidget(m_ricercaMobPage); // indice 30

    /* A-10 — CollabPage: BT Chat + PC Hub */
    m_collabPage = new CollabPage(m_ai, this);
    m_stack->addWidget(m_collabPage);     // indice 31

    /* A-7 — AppControllerPage: Anki FlashCard + TinyMCP client */
    m_appControllerPage = new AppControllerPage(m_ai, this);
    m_stack->addWidget(m_appControllerPage); // indice 32
    ...
    /* C4 — controlla share intent all'avvio (differito per attendere widget pronti) */
    QTimer::singleShot(0, this, &MainWindow::handleShareIntent);
}
...
void MainWindow::buildDrawer()
{
    ...
    const NavItem items[] = {
        ...
        { "\xf0\x9f\x94\x81",             "Pipeline AI",      m_idxPipeline      },
        { "\xf0\x9f\x8e\xaf",             "Sfida! Quiz",      m_idxSfida         },
        { "\xe2\x8f\xb0",                 "Cron Scheduler",   m_idxCron          },
        { "\xf0\x9f\x93\x9d",             "Editor + REPL",    m_idxEditor        },
        { "\xf0\x9f\x94\xac",             "Ricerca RAG",      m_idxRicercaMob    },
        { "\xf0\x9f\x94\x97",             "Collab BT+PC",     m_idxCollab        },
        { "\xf0\x9f\x95\xb9",             "App Controller",   m_idxAppController },
...
/* ══════════════════════════════════════════════════════════════
   C4 — handleShareIntent: processa ACTION_SEND in arrivo da
   altre app Android. Estrae testo o URI del file e naviga
   direttamente a FileAiPage (indice m_idxFileAi).
   ══════════════════════════════════════════════════════════════ */
void MainWindow::handleShareIntent()
{
#ifdef Q_OS_ANDROID
    /* Recupera l'Intent con cui l'Activity è stata avviata */
    ...
    static const QString kActionSend = "android.intent.action.SEND";
    if (actionStr != kActionSend) return;

    /* Prova prima EXTRA_TEXT (testo condiviso) */
    QJniObject extraText = QJniObject::fromString("android.intent.extra.TEXT");
    QJniObject textObj   = intent.callObjectMethod(
        "getStringExtra",
    ...
    QString sharedText;
    if (textObj.isValid()) {
        sharedText = textObj.toString();
    }

    /* Se non c'è testo prova EXTRA_STREAM (URI file) */
    QString sharedUri;
    if (sharedText.isEmpty()) {
        QJniObject extraStream = QJniObject::fromString("android.intent.extra.STREAM");
        QJniObject uriObj      = intent.callObjectMethod(
            "getParcelableExtra",
            "(Ljava/lang/String;)Landroid/os/Parcelable;",
            extraStream.object<jstring>());
        if (uriObj.isValid()) {
            QJniObject uriStr = uriObj.callObjectMethod(
                "toString", "()Ljava/lang/String;");
            if (uriStr.isValid())
                sharedUri = uriStr.toString();
        }
    }

    if (sharedText.isEmpty() && sharedUri.isEmpty()) return;

    /* Naviga a FileAiPage e passa il contenuto */
    onTabChanged(m_idxFileAi);
    if (m_fileAiPage) {
        if (!sharedText.isEmpty())
            m_fileAiPage->loadSharedText(sharedText);
        else if (!sharedUri.isEmpty())
            m_fileAiPage->loadSharedUri(sharedUri);
    }

    /* Consuma l'intent per evitare double-processing al resume */
    QJniObject emptyIntent("android/content/Intent", "()V");
    if (emptyIntent.isValid())
        activity.callMethod<void>("setIntent",
            "(Landroid/content/Intent;)V", emptyIntent.object());
#endif
}
