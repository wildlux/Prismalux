...
QWidget* ImparaPage::buildModelBar(QWidget* parent) {
    ...
    auto refreshModels = [=]{
        ...
        if (idx == 0) {
            ...
            /* llama-server */
            m_ai->setBackend(AiClient::LlamaServer, P::kLocalHost, P::kLlamaServerPort, "");
            m_ai->fetchModels();
        } else {
...
QWidget* ImparaPage::buildMenu() {
    ...
    auto* title = new QLabel("\xf0\x9f\x93\x9a  Impara & Sfidati", w); title->setObjectName("pageTitle");
    auto* sub   = new QLabel("Tutor interattivo \xc2\xb7 Sfida te stesso con quiz AI \xc2\xb7 Dashboard progressi \xe2\x80\x94 Oracolo di Prismalux", w);
    ...
    QList<Item> items = {
        {"\xf0\x9f\x8f\x9b\xef\xb8\x8f", "Tutor AI \xe2\x80\x94 Oracolo",
         "Spiegazioni personalizzate su qualsiasi materia. Matematica, informatica, fisica e altro.", 1},
        {"\xf0\x9f\x8e\xaf", "Sfida te stesso \xe2\x80\x94 Quiz AI",
         "Quiz a risposta multipla generati dall'AI con correzione immediata e spiegazione.", 2},
        {"📊", "Dashboard Statistica",
         "Analisi dei tuoi quiz: punteggi per materia, andamento nel tempo, sessioni recenti.", 3},
        {"📖", "Materie",
...
QWidget* ImparaPage::buildTutor() {
    ...
    auto* send = new QPushButton("Chiedi ▶", inRow); send->setObjectName("actionBtn");
    auto* stop = new QPushButton("⏹", inRow);
    ...
    auto sendFn = [=]{
        ...
        inp->clear(); send->setEnabled(false); stop->setEnabled(true);
    ...
    connect(m_ai, &AiClient::error, this, [=](const QString& e){
        m_tutorLog->append(QString("\n❌ %1").arg(e)); send->setEnabled(true); stop->setEnabled(false);
    });
...
QWidget* ImparaPage::buildQuiz() {
    ...
    auto* hdrL = new QHBoxLayout(hdrW); hdrL->setContentsMargins(0,0,0,0);
    auto* back = new QPushButton("← Torna", hdrW); back->setObjectName("actionBtn");
    auto* lbl  = new QLabel("\xf0\x9f\x8e\xaf  Sfida te stesso \xe2\x80\x94 Quiz AI", hdrW); lbl->setObjectName("pageTitle");
    hdrL->addWidget(back); hdrL->addWidget(lbl, 1);
    ...
    : QWidget(parent), m_ai(ai)
{
    ...
    connect(m_simulatorePage, &SimulatorePage::openMonitorRequested, this, [this]{
        /* Cerca il MonitorPanel tra i figli della MainWindow (creato lazy) */
        auto* mw = window();
        auto* panel = mw ? mw->findChild<MonitorPanel*>() : nullptr;
        if (!panel) {
            auto* hw = mw ? mw->findChild<HardwareMonitor*>() : nullptr;
            if (mw && hw) {
                panel = new MonitorPanel(m_ai, hw, mw);
