...
WHISPER_WIN_DIR="$C_SW/whisper_win"
...
if [ "$DO_BUILD_WHISPER" = "1" ]; then
    WHISPER_SRC="$C_SW/C_software/whisper.cpp"
...
[ "$DO_BUILD_WHISPER" = "1" ] && [ -f "$C_SW/C_software/whisper.cpp/build/bin/whisper-cli" ] && \
    echo -e "  ${G}whisper-cli${N}   $C_SW/C_software/whisper.cpp/build/bin/whisper-cli"
