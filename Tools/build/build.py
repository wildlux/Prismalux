...
# ── Costanti base ─────────────────────────────────────────────────────────────
ROOT    = Path(__file__).parent.parent.parent.resolve()  # Tools/build/ → Tools/ → root
GUI_DIR = ROOT / "gui"
...
def _win_find_qt6(msys2: Path | None) -> Path | None:
    ...
    toolchain = ROOT / "Tools" / "compile_win" / "toolchain" / "Qt6"
    if toolchain.exists():
...
def _win_find_cmake(qt: Path | None, msys2: Path | None) -> str:
    cmake_tool = ROOT / "Tools/compile_win/toolchain/cmake"
    portable = None
...
def _win_find_ninja(qt: Path | None, msys2: Path | None) -> str | None:
    return find_exe(
        ...
        ROOT / "Tools/compile_win/toolchain/ninja/ninja.exe",
        shutil.which("ninja"),
    )
...
def _win_copy_essential_dlls(qt: Path, build: Path) -> None:
    ...
    dlls = [
        ...
        # Qt WebEngine (necessario per KaTeX)
        "Qt6WebEngineCore.dll", "Qt6WebEngineWidgets.dll",
        "Qt6Quick.dll", "Qt6QuickWidgets.dll",
        # Runtime MinGW
...
def _win_copy_webengine(qt: Path, build: Path) -> None:
    """Copia QtWebEngineProcess.exe e resources/ per KaTeX (se non già presenti)."""
    proc_dst = build / "QtWebEngineProcess.exe"
    if not proc_dst.exists():
        search: list[Path] = [qt / "bin/QtWebEngineProcess.exe"]
        for msys in [Path("C:/msys64"), Path("C:/msys2"), Path("D:/msys64"), Path("D:/msys2")]:
            if msys.exists():
                search += [
                    msys / "ucrt64/lib/qt6/QtWebEngineProcess.exe",
                    msys / "ucrt64/bin/QtWebEngineProcess.exe",
                ]
                break
        for src in search:
            if src.exists():
                shutil.copy2(src, proc_dst)
                ok("QtWebEngineProcess.exe copiato")
                break
        else:
            warn("QtWebEngineProcess.exe non trovato — KaTeX potrebbe non funzionare")

    res_dst = build / "resources"
    if not res_dst.exists():
        res_search: list[Path] = [qt / "resources", qt / "bin/resources"]
        for msys in [Path("C:/msys64"), Path("C:/msys2"), Path("D:/msys64"), Path("D:/msys2")]:
            if msys.exists():
                res_search += [
                    msys / "ucrt64/share/qt6/resources",
                    msys / "ucrt64/lib/qt6/resources",
                ]
                break
        for src in res_search:
            if src.exists():
                shutil.copytree(src, res_dst, dirs_exist_ok=True)
                ok("resources/ WebEngine copiato")
                break


def _win_verify_deploy(build: Path) -> None:
    """Stampa tabella di verifica file essenziali dopo windeployqt."""
    essential = [
        ("Prismalux_GUI.exe",       "eseguibile principale"),
        ("Qt6Core.dll",             "Qt core"),
        ("Qt6Gui.dll",              "Qt GUI"),
        ("Qt6Widgets.dll",          "Qt Widgets"),
        ("Qt6Network.dll",          "Rete / auto-update"),
        ("Qt6Sql.dll",              "GraphMemory SQLite"),
        ("Qt6WebEngineCore.dll",    "KaTeX / WebEngine"),
        ("Qt6WebEngineWidgets.dll", "KaTeX / WebEngine"),
        ("QtWebEngineProcess.exe",  "KaTeX / WebEngine process"),
        ("libgcc_s_seh-1.dll",      "runtime MinGW"),
        ("libstdc++-6.dll",         "runtime MinGW"),
        ("libwinpthread-1.dll",     "runtime MinGW"),
        ("platforms/qwindows.dll",  "plugin finestra Windows"),
    ]
    step("Verifica deploy:")
    missing: list[str] = []
    for fname, desc in essential:
        if (build / fname).exists():
            ok(f"  {fname:<38} ✓")
        else:
            warn(f"  {fname:<38} ✗  ({desc})")
            missing.append(fname)
    print()
    if missing:
        warn(f"{len(missing)} file mancanti — l'app potrebbe non avviarsi su PC senza Qt installato.")
    else:
        ok(f"Tutti {len(essential)} file essenziali presenti.")
...
def main_windows(version: str) -> int:
    ...
    _win_copy_webengine(qt, BUILD_DIR)

    _win_verify_deploy(BUILD_DIR)
