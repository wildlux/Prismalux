...
_ROOT_PRISMALUX = os.path.dirname(os.path.dirname(ROOT_STUDIO))  # llama_cpp_studio → Ottimizzazioni_Avanzate → Prismalux
if os.path.join(_ROOT_PRISMALUX, "core") not in sys.path:
    sys.path.insert(0, os.path.join(_ROOT_PRISMALUX, "core"))
...
def _cerca_gguf() -> list:
    ...
    def _scan(cartella: Path, max_depth: int) -> None:
        """Scansione con profondità massima per evitare blocchi su home grandi."""
        try:
            for item in cartella.iterdir():
                if item.is_file() and item.suffix.lower() == ".gguf":
                    ...
                    _scan(item, max_depth - 1)
        except (PermissionError, OSError):
            pass

    # Cartella progetto: ricerca completa (è nostra, piccola)
    try:
        for f in Path(DIR_MODELS).rglob("*.gguf"):
            if f.is_file():
                try:
                    trovati.append({
                        "nome":     f.name,
    ...
    # Cartelle utente: profondità limitata (no rglob illimitato → non blocca)
    _scan(Path.home() / "Downloads", max_depth=1)
    _scan(Path.home() / "Modelli",   max_depth=1)
    _scan(Path.home() / "Models",    max_depth=1)
    # Hugging Face hub: struttura fissa hub/models--org--name/snapshots/hash/file.gguf
    _scan(Path.home() / ".cache" / "huggingface" / "hub", max_depth=4)
...
def avvia_modello(con: "Console", cfg: dict) -> dict:
    ...
    # ── Pannello configurazione + modalità ───────────────────────────────
    os.system("cls" if sys.platform == "win32" else "clear")
    stampa_header(con)
    stampa_breadcrumb(con, "🍺 Prismalux › ⚙️ llama.cpp Studio › 🚀 Avvia")
    ...
    while True:
        sx2 = Text()
        sx2.append(f"\n  Modello:  {modello['nome']}  ({size_gb:.1f} GB)\n\n", style="bold white")
        sx2.append("  Parametri di avvio:\n\n", style="dim")
        sx2.append(f"  Threads     : ", style="dim")
        sx2.append(f"{n_threads}\n", style="bold cyan")
        sx2.append(f"  GPU layers  : ", style="dim")
        sx2.append(f"{n_gpu_layers}",  style="bold green")
        sx2.append("  (0 = solo CPU)\n", style="dim")
        sx2.append(f"  Context     : ", style="dim")
        sx2.append(f"{ctx_size} token\n\n", style="bold cyan")
        sx2.append("  ────────────────────────────────\n\n", style="dim")
        sx2.append("  1.  💬  Chat interattiva\n", style="bold white")
        sx2.append("          Sessione diretta nel terminale\n\n", style="dim")
        sx2.append("  2.  🌐  Server API  (localhost:8080)\n", style="bold white")
        sx2.append("          Compatibile con OpenAI API\n\n", style="dim")
        sx2.append("  P.  🔧  Modifica parametri\n", style="bold yellow")
        sx2.append("          (threads / gpu_layers / ctx_size)\n\n", style="dim")
        sx2.append("  0.  ←   Annulla\n", style="dim")

        dx2 = Text()
        dx2.append("  ─────────────\n\n", style="dim")
        dx2.append("  1  chat\n",      style="bold white")
        dx2.append("  2  server\n",    style="bold white")
        dx2.append("  P  parametri\n", style="bold yellow")
        dx2.append("  0  annulla\n",   style="dim")

        grid2 = Table.grid(expand=True)
        grid2.add_column(ratio=3)
        grid2.add_column(min_width=18, ratio=1)
        grid2.add_row(
            Panel(sx2, title=f"[bold green]🚀 Avvia: {modello['nome']}[/]",
                  border_style="green", padding=(1, 2)),
            Panel(dx2, border_style="green", padding=(1, 1)),
        )
        con.print(grid2)
        con.print(Rule(style="dim"))
        sys.stdout.write("  Scelta [1/2/P/0] ❯ ")
        sys.stdout.flush()
        ...
        if modo == "P":
            # ── Modifica parametri ────────────────────────────────────────
            con.print("\n  [dim]Lascia vuoto per mantenere il valore corrente.[/]\n")
            try:
                raw = input(f"  Threads [{n_threads}]: ").strip()
                if raw.isdigit():
                    n_threads = max(1, int(raw))
                raw = input(f"  GPU layers [{n_gpu_layers}]: ").strip()
                if raw.isdigit():
                    n_gpu_layers = int(raw)
                raw = input(f"  Context size [{ctx_size}]: ").strip()
                if raw.isdigit():
                    ctx_size = max(512, int(raw))
            except (EOFError, KeyboardInterrupt):
                pass
            os.system("cls" if sys.platform == "win32" else "clear")
            stampa_header(con)
...
def menu_studio() -> None:
    ...
    while True:
        ...
        if con:
            ...
            compilato  = cfg.get("compilato", False)
            hw_cache   = cfg.get("hardware", {})
            cpu_str    = hw_cache.get("cpu_nome", "?")[:28] if hw_cache else "non rilevato"
            gpu_str    = (hw_cache.get("gpu_nvidia") or hw_cache.get("gpu_amd") or "—") if hw_cache else "—"
            ram_str    = f"{hw_cache.get('ram_gb', 0):.0f} GB" if hw_cache else "?"

            banner = Text()
            banner.append("  llama.cpp : ", style="dim")
            if compilato:
                banner.append("✓ Compilato\n", style="bold green")
            else:
                banner.append("⚠  Non compilato — esegui prima la voce 1\n", style="bold yellow")
            banner.append(f"  Modello   : {moddef_str}\n", style="dim")
            banner.append(f"  Hardware  : {cpu_str}  |  GPU: {gpu_str}  |  RAM: {ram_str}\n", style="dim")
            if not compilato:
                banner.append("\n  👉 Suggerimento: compila una volta sola (voce 1),", style="dim")
                banner.append(" poi potrai avviare qualsiasi .gguf con la voce 3.\n", style="dim")
            con.print(Panel(banner, border_style="dim magenta", padding=(0, 1)))
            con.print()

            # ── Voci menu ─────────────────────────────────────────────────────
            sx = Text()
            sx.append("  1.  🔨  Compila llama.cpp\n", style="bold magenta")
            sx.append("          (git clone + cmake, una volta sola)\n\n", style="dim")
            sx.append("  2.  📂  Gestisci Modelli GGUF\n", style="bold green")
            sx.append("          (cerca .gguf, imposta modello default)\n\n", style="dim")
            sx.append("  3.  🚀  Avvia Modello\n", style="bold magenta")
            sx.append("          (chat o server :8080, parametri modificabili)\n\n", style="dim")
            sx.append("  4.  ℹ️   Info Hardware\n", style="bold green")
            sx.append("          (CPU, SIMD, GPU, RAM, flag CMake consigliati)\n\n", style="dim")
            sx.append("  0.  ←   Torna al menu Prismalux\n", style="dim")
            ...
            dx.append("  2  modelli\n",   style="dim")
            dx.append("  3  avvia\n",     style="bold magenta")
            dx.append("  4  hardware\n",  style="dim")
            dx.append("  0  torna\n",     style="dim")

            grid = Table.grid(expand=True)
            grid.add_column(ratio=3)
            grid.add_column(min_width=18, ratio=1)
            grid.add_row(
                Panel(sx, title="[bold magenta]⚙️  llama.cpp Studio[/]",
                      border_style="bold magenta", padding=(1, 2)),
                Panel(dx, border_style="magenta", padding=(1, 1)),
            )
            con.print(grid)
            con.print(Rule(style="dim"))
