...
_ROOT = os.path.dirname(os.path.dirname(_CART))  # cython_studio → Ottimizzazioni_Avanzate → Prismalux
...
# ─────────────────────────────────────────────────────────────────────────────
# Flusso guidato: seleziona → converti → compila (tutto in un unico percorso)
# ─────────────────────────────────────────────────────────────────────────────

def _flusso_compila() -> None:
    """
    Flusso guidato completo: seleziona file → converti in .pyx → compila.
    L'utente non deve sapere nulla di Cython — il menu fa tutto.
    """
    # Passo 1: seleziona file .py
    ...
    console.clear()
    stampa_header(console, risorse())
    console.print(Rule(f"[cyan]File selezionato: {_rel(path)}[/]"))

    # Passo 2: mostra analisi suggerimenti (sintetica)
    codice = _leggi_file(path)
    righe = codice.splitlines()
    suggerimenti = []
    for n, riga in enumerate(righe, start=1):
        s = riga.strip()
        if s.startswith("def ") and "self" not in s and "cls" not in s:
            suggerimenti.append(f"  riga {n:4}: {s[:70]}")
        elif s.startswith("for ") and " in range(" in s:
            suggerimenti.append(f"  riga {n:4}: loop → cdef int (ottimizzabile)")

    if suggerimenti:
        t = Text("\n")
        t.append("  Funzioni/loop ottimizzabili trovati:\n\n", style="bold yellow")
        for sg in suggerimenti[:8]:
            t.append(f"{sg}\n", style="dim")
        if len(suggerimenti) > 8:
            t.append(f"  … e altri {len(suggerimenti)-8} punti\n", style="dim")
        console.print(Panel(t, title="[yellow]🔍 Analisi rapida[/]",
                            border_style="yellow", padding=(0, 1)))
    else:
        console.print("\n  [green]Nessuna ottimizzazione automatica ovvia — procedo comunque.[/]\n")

    # Passo 3: converti .py → .pyx
    dest = _converti_pyx(path)
    if not dest:
        input("  [INVIO] ")
        return

    console.print("\n  [dim]Puoi modificare il .pyx prima di compilare per aggiungere cdef/cpdef.[/]")
    try:
        if input("  Aprire il .pyx con nano per modificarlo? [s/N] ").strip().lower() == "s":
            editor = shutil.which("nano") or shutil.which("notepad")
            if editor:
                subprocess.run([editor, dest])
    except (KeyboardInterrupt, EOFError):
        pass

    # Passo 4: compila
    console.print()
    ok = _compila_pyx(dest)
    if ok:
        console.print("\n  [bold green]✅ Tutto fatto! Puoi importare il modulo .so/.pyd nel tuo codice.[/]")
    input("\n  [INVIO] per continuare ")


# ─────────────────────────────────────────────────────────────────────────────
# Menu principale Cython Studio (semplificato)
# ─────────────────────────────────────────────────────────────────────────────
...
def menu_cython() -> None:
    while True:
        ...
        stato = "[green]✓ pronto[/]" if _cython_disponibile() else "[yellow]auto-installa[/]"

        # Banner integrazione Multi-Agente
        banner = Text()
        banner.append("  💡 Integrato nel ", style="dim")
        banner.append("Multi-Agente", style="bold cyan")
        banner.append(": alla fine di ogni task ti viene chiesto\n", style="dim")
        banner.append("     automaticamente se vuoi compilare il codice generato.\n", style="dim")
        banner.append("     Puoi usare questo menu anche su qualsiasi tuo file Python.", style="dim")
        console.print(Panel(banner, border_style="dim blue", padding=(0, 1)))
        console.print()

        t = Text()
        t.append(f"      Cython: {stato}\n\n", style="")
        t.append("      1.  🚀  Compila un file Python → .so/.pyd\n",    style="bold cyan")
        t.append("              (seleziona → analizza → converti → compila)\n\n", style="dim")
        t.append("      2.  ⚡  Benchmark: Python vs Cython\n",           style="bold yellow")
        t.append("              (misura il guadagno di velocità reale)\n\n", style="dim")
        t.append("      3.  📋  Risultati compilati\n\n",                 style="bold white")
        t.append("      4.  🗑️   Pulisci file compilati\n\n",             style="bold red")
        t.append("      5.  📚  Guida rapida Cython\n\n",                 style="bold green")
        t.append("      0.  ←   Torna al menu principale\n",              style="dim")
        console.print(Panel(
            t,
            title="[bold yellow]⚡ Cython Studio — Python → C compilato[/]",
            border_style="yellow",
        ))
