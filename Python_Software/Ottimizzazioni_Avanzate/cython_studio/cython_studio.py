...
_ROOT_MA  = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # cython_studio → Ottimizzazioni_Avanzate → Prismalux
...
def installa_cython() -> bool:
    """Installa Cython e setuptools via pip se non presenti. Ritorna True se pronto."""
    pacchetti_da_installare = []
    if importlib.util.find_spec("Cython") is None:
        pacchetti_da_installare.append("cython")
    if importlib.util.find_spec("setuptools") is None:
        pacchetti_da_installare.append("setuptools")

    if not pacchetti_da_installare:
        return True

    _print(f"Installazione: {', '.join(pacchetti_da_installare)}...", "yellow")
    ok = subprocess.run(
        [sys.executable, "-m", "pip", "install", "--quiet"] + pacchetti_da_installare,
        capture_output=True,
    ).returncode == 0
    if ok:
        _print("Dipendenze installate correttamente.", "green")
    else:
        _print(
            f"Errore installazione. Riprova manualmente: "
            f"pip install {' '.join(pacchetti_da_installare)}",
            "red",
        )
    return ok
...
7. CRITICO — tipo ritorno in cpdef/cdef va PRIMA del nome, stile C: `cpdef double funzione(int n, double x):` — MAI usare `-> tipo` in cpdef/cdef (è sintassi Python pura, Cython la rifiuta)
...
11. Per funzioni `def` normali (non cpdef/cdef) puoi usare `-> tipo` opzionalmente
...
Scrivi SOLO il codice .pyx completo e funzionante:"""
    ...
    try:
        ...
        codice_pyx = match.group(1).strip() if match else risposta.strip()
        # Sanitizza: rimuove `-> tipo` da cpdef/cdef (sintassi Python invalida in Cython)
        return _sanitizza_pyx(codice_pyx)

    except Exception as e:
        _print(f"Errore Ollama: {e}", "red")
        return ""

# ── 2b. Sanitizza il .pyx generato dall'AI ────────────────────────────────────

def _sanitizza_pyx(codice: str) -> str:
    """
    Rimuove annotazioni Python-style `-> tipo` da linee cpdef/cdef,
    che causano errori di compilazione Cython.
    Es: `cpdef list funzione(int n) -> list:` → `cpdef list funzione(int n):`
    """
    import re as _re
    righe = codice.splitlines()
    righe_corrette = []
    for riga in righe:
        stripped = riga.lstrip()
        # cpdef o cdef con -> annotation: rimuovi `-> qualcosa` prima del `:`
        if stripped.startswith(("cpdef ", "cdef ")) and "->" in riga:
            riga = _re.sub(r"\s*->\s*[\w\[\], ]+(?=\s*:)", "", riga)
...
def _crea_setup_py_in_dir(nome_pyx: str, build_dir: str) -> str:
    """
    Crea setup_cython.py nella cartella di build (NON in esportazioni).
    nome_pyx è il basename del file .pyx (relativo a build_dir).
    """
    path_setup = os.path.join(build_dir, "setup_cython.py")
...
def compila_pyx(path_pyx: str) -> tuple[bool, str, str]:
    """
    Compila il file .pyx usando una cartella temporanea isolata.
    setup_cython.py e gli artefatti di build NON finiscono in esportazioni.
    Ritorna (successo, output_log, path_modulo_compilato).
    """
    import tempfile
    cartella_pyx = os.path.dirname(path_pyx)
    nome_pyx     = os.path.basename(path_pyx)
    nome_base    = os.path.splitext(nome_pyx)[0]

    _print("Compilazione in corso...", "cyan")

    # Controlla compilatore C su Windows
    ...
    build_dir = tempfile.mkdtemp(prefix="prismalux_cython_")
    try:
        # Copia .pyx nella build dir per evitare artefatti in esportazioni
        pyx_in_build = os.path.join(build_dir, nome_pyx)
        shutil.copy2(path_pyx, pyx_in_build)

        path_setup = _crea_setup_py_in_dir(nome_pyx, build_dir)
        ...
        # Cerca il .so/.pyd compilato nella build dir
        ext = "pyd" if sys.platform == "win32" else "so"
        trovati = _glob_mod.glob(os.path.join(build_dir, f"{nome_base}*.{ext}"))

        modulo_compilato = ""
        if trovati:
            so_src = trovati[0]
            so_dst = os.path.join(cartella_pyx, os.path.basename(so_src))
            shutil.copy2(so_src, so_dst)
            modulo_compilato = so_dst
...
def pipeline_cython(
    ...
    # Retry automatico: se AI ha generato il .pyx ma la compilazione fallisce,
    # riprova con il .pyx base (senza AI) prima di arrendersi
    if not successo and usa_ai:
        _print("⚠  .pyx AI fallito — riprovo con .pyx base (senza AI)...", "yellow")
        codice_pyx_base = genera_pyx_base(codice_py, nome_modulo)
        with open(pyx_path, "w", encoding="utf-8") as f:
            f.write(codice_pyx_base)
        successo, log, so_path = compila_pyx(pyx_path)
        if successo:
            _print("✓ Compilazione riuscita con .pyx base.", "green")
            usa_ai = False
        else:
            _print("✗ Anche il .pyx base ha fallito. Controlla il log.", "red")
