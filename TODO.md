...
| D-24 | 🟡 | **Normalizzazione numeri/formati IT** — virgola decimale ("3,5"→"3.5"), separatore migliaia ("100.000 euro"), date miste (15/3/90, "15 marzo 1990") in un unico punto prima della catena (verificare cosa copre già `normalizeNumbers`) | `main_ai_math.cpp` | ⬜ |

### Stadio B — Nuove guardie zero-LLM

| # | Priorità | Descrizione | File | Stato |
|---|----------|-------------|------|-------|
| D-25 | 🟡 | **Cache risposte esatte** — hash query normalizzata → risposta precedente, con link "🔄 rigenera" nella bolla per bypassare. Query ripetute identiche = zero token, utilissima anche per i test manuali ripetuti | `main_ai_pipeline.cpp` + QHash/file in `~/.prismalux/` | ⬜ |
| D-26 | 🟡 | **Lookup memoria personale zero-LLM** — se la query matcha per keyword una voce di `KNOWLEDGE_USER/user_knowledge.md` (già letto con cache 30s da `P::readUserKnowledge()`), rispondi direttamente dal file senza modello ("qual è l'IP della telecamera?") | `main_ai_tools.cpp` (nuova `_inject_knowledge`) | ⬜ |
...
| # | Priorità | Descrizione | File | Stato |
|---|----------|-------------|------|-------|
| D-27 | 🟡 | **Dominio → modello (SmartRouter)** — `detectQueryDomain` e SmartRouter API esistono già in `AiClient` (test CAT-B/C `test_ai_client`): mapparle sui modelli installati — codice→deepseek-coder, immagine allegata→llama3.2-vision, resto→modello leggero. Opt-in (checkbox "routing automatico") per non scavalcare la scelta manuale del combo | `ai_client.cpp`, `main_ai_pipeline.cpp` | ⬜ |
...
| D-29 | 🟡 | **Pre-selezione function tools per categoria** — filtro keyword→categoria (finanza/rete/file/algoritmi) che espone al modello SOLO i tool pertinenti alla query: riduce i token fissi delle definizioni tool per turno, sensibile sui modelli locali. Diventa più importante man mano che D-33 aggiunge tool | `main_ai_tools.cpp` (builder array tools) | ⬜ |

### Stadio D — Contesto condizionale

| # | Priorità | Descrizione | File | Stato |
|---|----------|-------------|------|-------|
| D-30 | 🟡 | **RAG gating via embedding** — oggi il RAG globale è iniettato sempre (sessione 2026-06-10c): check di similarità embedding query↔indice (`fetchEmbedding` esiste già) e inietta SOLO se sopra soglia — meno rumore e meno token sulle query che col RAG non c'entrano | `ai_client.cpp` (injection RAG) | ⬜ |
