...
| D-17 | 🟡 | ⬜ Aperto — **riuso algoritmi del Simulatore** (`main_simulator.h`, >90 algoritmi già implementati) per generare altre domande classiche zero-LLM in modo automatizzato. Verificato concretamente: `genGCD(a,b)` e `genPrimeFactors(n)` (in `main_sim_sorting.cpp`) restituiscono `QVector<AlgoStep>` il cui **ultimo elemento ha già un `.msg` testuale pulito con la risposta finale** (es. `genPrimeFactors(360)` → ultimo `.msg` = `"360 = 2×2×2×3×3×5"`) — non serve reimplementare l'algoritmo, basta `steps.last().msg`. Ostacolo: sono metodi *membro* di `SimulatorePage` (serve un'istanza pesante con tutta la UI), vanno resi **`static`** — non toccano mai `this`/membri, refactor piccolo e sicuro (verificare caso per caso prima di convertire). Candidati pronti in categoria CAT_MATH (tutti "un numero in ingresso → messaggio pulito in uscita"): `genGCD`/`genExtGCD`, `genFastPow`, `genPrimeFactors`, `genMillerRabin` (primalità), `genPascalTriangle`, `genFibonacciDP`, `genCatalan`, `genCollatz`, `genKaratsuba`, `genSieve`/`genSieveSundaram`; + `genTowerOfHanoi` (mosse minime) fuori categoria. Non implementato: tocca codice esistente testato, da confermare prima di procedere | `main_simulator.h`, `main_sim_sorting.cpp`, `main_sim_graph.cpp`, `main_sim_misc.cpp` (rendere static) + nuovo `_inject_algo()` in `main_ai_tools.cpp` | ⬜ |
