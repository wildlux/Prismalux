...
| D-13 | 🟢 | ⬜ Aperto — quando l'utente chiede "cosa sai fare?" (`_inject_help()`, D-11), mostrare ANCHE esempi dei grafici disponibili: elencare tutte le 17 tipologie di `ChartType` (`main_graph.h`: Cartesian, Pie, Histogram, ScatterXY, Graph, Scatter3D, Graph3D, SmithPrime, MathConst, Line, Polar, Radar, Bubble, Heatmap, Candlestick, Area, Waterfall) e mostrare un esempio concreto **scelto a caso** (`QRandomGenerator`) ad ogni richiesta, non sempre lo stesso — es. "prova: disegna un grafico a torta con [dati esempio]" cambia tipologia/dati ogni volta che si chiede aiuto. Da valutare se generare anche l'anteprima inline (richiede integrazione con `GraficoCanvas`/`FormulaParser`, non solo testo) | `main_ai_tools.cpp` (`_inject_help`), `main_graph.h` | ⬜ |
