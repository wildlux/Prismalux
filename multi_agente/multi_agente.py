...
CARTELLA_OUTPUT = os.path.dirname(os.path.abspath(__file__))
...
def controlla_istanza(porta: int) -> bool:
    ...
    try:
        r = requests.get(f"http://localhost:{porta}/api/tags", timeout=2)
...
def _offri_compilazione_cython(path_py: str, modello: str, porta: int) -> None:
    """
    Dopo la pipeline multi-agente, offre di compilare il .py con Cython.
    """
    ...
    if _RICH_OK and _rich:
        ...
        t = _TCy()
        t.append("  Il codice e stato salvato. Vuoi compilarlo con Cython\n")
        t.append("  per massimizzare le prestazioni in C?\n\n")
        t.append("  [S]  Si — compila ora con Cython\n",          style="bold cyan")
        t.append("  [N / Invio]  No — salta la compilazione\n",   style="dim")
        _rich.print(_PCy(t, title="[bold cyan]⚡ Bridge Cython — Ottimizzazione C[/]",
                         border_style="cyan", padding=(0, 1)))
    else:
        print("\n  ⚡ Vuoi compilare il codice con Cython (C)? [S/n]")
...
def avvia_menu() -> None:
    ...
    try:
        ...
        from check_deps import stampa_header as _stampa_h, stampa_breadcrumb as _stampa_bc
        ...
        def _leggi_tasto_raw_ma() -> str:
            ...
            if not _HAS_TERMIOS:
                # Windows: legge una riga intera
                s = input("  > ").strip().upper()
                if s in ('T', 'TAB'):
                    return 'TAB'
                if s in ('', '\x7f', '\x08'):
                    return 'BACKSPACE'
                return 'ENTER' if not s else s[:1]
            fd = sys.stdin.fileno()
            old = termios.tcgetattr(fd)
            try:
                tty.setraw(fd)
        ...
        def _disegna_menu_ma(modalita_online: bool, gpu_attive: dict | None = None) -> None:
            ...
            _stampa_h(_con)
            _stampa_bc(_con, "Prismalux › 🤖 Multi-Agente AI")
        ...
        def _scheda_seleziona_modelli_ma(memoria_in: dict, solo_locali: bool) -> tuple[dict, dict, str]:
            ...
            def _ridisegna(msg: str = "") -> None:
                ...
                _stampa_h(_k2)
                _stampa_bc(_k2, "Prismalux › 🤖 Multi-Agente AI › ⚙️  Modelli")
                sx2 = _Tex_ma()
                sx2.append("  Modelli Ollama disponibili:\n\n", style="dim")
                for i, m in enumerate(modelli_info, 1):
                    gb = f"{m['size_gb']:.1f}GB" if m["size_gb"] > 0 else "cloud"
                    sx2.append(f"  {i:2}. {m['nome']:<34} {gb}\n", style="dim")
                sx2.append("\n  Assegnazione agenti:\n\n", style="bold")
                for idx_r, ruolo in enumerate(_RL, 1):
                    icona, _ = _RUOLI_INFO[ruolo]
                    mod  = scelti[ruolo]
                    d_ag = _dispositivi_agenti[ruolo]
                    coder = " ← coder ✓" if any(k in mod for k in ("coder", "code")) else ""
                    saved = " 💾" if mod == pref_modelli.get(ruolo) else ""
                    d_col = "green" if d_ag == "GPU" else "yellow"
                    sx2.append(f"  [{idx_r}] {icona:<22} ", style="cyan")
                    sx2.append(f"{_disp_icon(d_ag)} ", style=d_col)
                    sx2.append(f"{mod}{coder}{saved}\n", style="cyan")
                # conta GPU / CPU separati
                n_gpu = sum(1 for d in _dispositivi_agenti.values() if d == "GPU")
                n_cpu = sum(1 for d in _dispositivi_agenti.values() if d == "CPU")
                sx2.append("\n  Distribuzione: ", style="dim")
                if n_cpu > 0 and n_gpu > 0:
                    sx2.append(f"🎮 {n_gpu} agenti GPU  +  🖥 {n_cpu} agenti CPU (parallelo)\n",
                               style="bold green")
                elif n_cpu == 0:
                    sx2.append(f"🎮 tutti su GPU — {_GP_EMOJI} {_GP_NOME} ({_GP_ACC})\n", style="green")
                else:
                    sx2.append("🖥  tutti su CPU — nessuna GPU\n", style="yellow")
                sx2.append(f"  💾 Salvato: {disp_salvato}\n", style="dim")
                if msg:
                    sx2.append(f"\n  {msg}\n", style="bold yellow")
                dx2 = _Tex_ma()
                dx2.append("  ─────────────────\n\n", style="dim")
                dx2.append("  1–5   modello\n",        style="dim")
                dx2.append("  G     tutti → GPU\n",    style="dim")
                dx2.append("  C     tutti → CPU\n",    style="dim")
                dx2.append("  M     GPU/CPU mix\n",    style="dim")
                dx2.append("  INVIO conferma\n",       style="bold green")
                dx2.append("  0 / ⌫  torna\n",         style="dim")
                body2 = _Tab_ma.grid(expand=True)
                body2.add_column(ratio=3)
                body2.add_column(min_width=24, ratio=1)
                body2.add_row(
                    _RichPanel(sx2, title="[bold cyan]⚙️  Modelli & Dispositivo[/]",
                               border_style="bold cyan", padding=(1, 2)),
                    _RichPanel(dx2, border_style="cyan", padding=(1, 1)),
                )
                _k2.print(body2)
            ...
            while True:
                ...
                if val == 'ENTER':
                    ...
                    icona_rc, _ = _RUOLI_INFO[ruolo_da_cambiare]
                    # Selezione modello (input testuale multi-cifra)
                    _ridisegna(f"Seleziona n° modello per {icona_rc} (1–{len(modelli_info)}):")
                    sys.stdout.write("  n° ❯ ")
                    sys.stdout.flush()
                    try:
                        n = int(input("").strip()) - 1
                        if 0 <= n < len(nomi):
                            scelti[ruolo_da_cambiare] = nomi[n]
                            # Selezione dispositivo per questo agente
                            _ridisegna(
                                f"✓ {ruolo_da_cambiare} → {nomi[n]}  "
                                f"Dispositivo [G=GPU / C=CPU / Invio=mantieni]:"
                            )
                            sys.stdout.write("  ❯ ")
                            sys.stdout.flush()
                            d_in = input("").strip().upper()
                            if d_in == "G":
                                _dispositivi_agenti[ruolo_da_cambiare] = "GPU"
                                _ridisegna(f"✓ {icona_rc} → {nomi[n]} 🎮 GPU")
                            elif d_in == "C":
                                _dispositivi_agenti[ruolo_da_cambiare] = "CPU"
                                _ridisegna(f"✓ {icona_rc} → {nomi[n]} 🖥 CPU")
                            else:
                                _ridisegna(f"✓ {ruolo_da_cambiare} → {nomi[n]}")
                        else:
                            _ridisegna("⚠️  Numero fuori range.")
                    except (ValueError, EOFError):
                        _ridisegna("⚠️  Input non valido.")
                else:
        ...
        # ── Scheda 3: Task input ────────────────────────────────────────────────
        def _disegna_task_ma(memoria_in: dict, modelli: dict, porte: dict, dispositivo: str) -> None:
            os.system("cls" if sys.platform == "win32" else "clear")
            _c3 = _sh_ma.get_terminal_size(fallback=(120, 40)).columns
            _k3 = _Con_ma(width=_c3)
            _stampa_h(_k3)
            _stampa_bc(_k3, "Prismalux › 🤖 Multi-Agente AI › 📝 Task")
            _RL3 = ["ricercatore", "pianificatore", "programmatore", "tester", "ottimizzatore"]
            sx3 = _Tex_ma()
            sx3.append("  Configurazione pronta:\n\n", style="bold green")
            for ruolo in _RL3:
                icona, _ = _RUOLI_INFO[ruolo]
                mod   = modelli.get(ruolo, "?")
                coder = " ✓" if any(k in mod for k in ("coder", "code")) else ""
                sx3.append(f"  {icona:<22} {mod}{coder}\n", style="cyan")
            sx3.append("\n  Dispositivi:\n", style="dim")
            for ruolo_d in _RL3:
                porta_d = porte.get(ruolo_d, PORTA_NVIDIA)
                d_icn   = "🎮" if porta_d == PORTA_NVIDIA else "🖥"
                d_lab   = _GP_NOME if porta_d == PORTA_NVIDIA else "CPU"
                d_col   = "green" if porta_d == PORTA_NVIDIA else "yellow"
                icona_d, _ = _RUOLI_INFO[ruolo_d]
                sx3.append(f"  {icona_d:<22} ", style="cyan")
                sx3.append(f"{d_icn} {d_lab}\n", style=d_col)
            task_prec = memoria_in.get("task_eseguiti", [])
            if task_prec:
                sx3.append("\n  Ultimi task:\n", style="dim")
                for t in reversed(task_prec[-3:]):
                    v = "✅" if t.get("verdict") == "PASSATO" else "❌"
                    sx3.append(f"  {v} {t['task'][:52]}\n", style="dim")
            dx3 = _Tex_ma()
            dx3.append("  ─────────────────\n\n", style="dim")
        ...
        # ── Scheda 3: Task input ───────────────────────────────────────────────
        _disegna_task_ma(memoria, modelli, porte, dispositivo)
        task = input("").strip()
        if task == "0":
            print("\n  Ritorno al menu principale...")
            sys.exit(2)
